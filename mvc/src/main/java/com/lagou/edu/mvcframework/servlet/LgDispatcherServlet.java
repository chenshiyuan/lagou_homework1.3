...
//        method.invoke() //
        ...
        if (!doSecurityCheck(handler,req)){
            try {
                resp.getWriter().write("<html>\n" +
                        "<body>\n" +
                        "<h2>You have no access right!</h2>\n" +
                        "</body>\n" +
                        "</html>");
            } catch (IOException e) {
                e.printStackTrace();
    ...
    private boolean doSecurityCheck(Handler handler, HttpServletRequest req) {
